...
async function migrateExistingContent() {
  ...
  for (const userDoc of verifiedSnapshot.docs) {
    ...
    if (!username) {
      console.log(`Skipping ${uid}: no username`);
      continue;
    }
    
    console.log(`Processing user ${uid} with username: ${username}`);
    ...
    console.log(`Updated ${gamesUpdated} games for ${username}`);
    ...
    console.log(`Updated ${articlesUpdated} articles for ${username}`);
  }
