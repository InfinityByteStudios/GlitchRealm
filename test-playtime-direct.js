    ...
    return batch.commit()
        .then(() => {
            ...
            Object.values(games).forEach(game => {
                ...
                console.log(`   - ${game.gameName}: ${hours} hours`);
            });
...
function resetPlaytimeData() {
    ...
    window.firestoreGetDocs(window.firestoreCollection(db, 'playtime', userId, 'games'))
        ...
        .then(() => {
            console.log('✅ Legacy playtime data has been cleared');
            console.log('🔄 Refresh the page to see the updated playtime data');
        })
        .catch(error => {
...
function addPlaytimeManually() {
    ...
    if (window.firebase && window.firebase.firestore) {
        ...
        // Commit all changes
        batch.commit()
            .then(() => console.log('Manual playtime data added to both global document and individual game documents!'))
...
console.log('For manual testing: addPlaytimeManually()');
